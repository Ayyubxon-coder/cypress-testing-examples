...
describe('Initial Page', () => {
  ...
  it('should navigate to "/sign-in" when you click the "Sign In" button', () => {
    ...
    cy.location('pathname').should('contain.text', '/sign-in'); // you can change it to should('equal', '/echo-chamber/sign-in')
  });

  it('should navigate to "/sign-up" when you click the "Sign Up" button', () => {
    cy.get('[data-test="sign-up"]').click();
    cy.location('pathname').should('contain.text', '/sign-up'); // you can change it to should('equal', '/echo-chamber/sign-up')
  });
});
